...
def verify_logic(
    ...
    try:
        ...
        updated_code = verify_logic_with_llm(
        ...
        if not updated_code or len(updated_code.strip()) == 0:
            logger.info(f"No logic issues found in component: {route_path}")
            return False

        # Write the updated code back to the file
        if write_file(str(react_component_path), updated_code):
            logger.info(f"Updated logic in component: {route_path}")
            return True
        else:
        ...
        logger.error(
            f"Error verifying logic for route {route.get('path', '')}: {str(e)}"
        )
        return False
...
def verify_logic_with_llm(
    ...
    try:
        ...
        prompt = PROMPT_FOR_LOGIC_VERIFICATION(

...
from prompt.code_verification import PROMPT_FOR_API_CALLS_VERIFICATION
...
def verify_api_calls(
    ...
    """
    Verify the API calls of a converted React component against the original Angular controller.
    The function will check for missing API calls, unnecessary API calls, and other API call issues.
    ...
    try:
        ...
        # Call LLM to verify the API calls
        updated_code = verify_api_calls_with_llm(
        ...
        if not updated_code or len(updated_code.strip()) == 0:
            logger.info(f"No API call issues found in component: {route_path}")
            return False

        # Write the updated code back to the file
        if write_file(str(react_component_path), updated_code):
            logger.info(f"Updated API calls in component: {route_path}")
            return True
        else:
        ...
        logger.error(
            f"Error verifying API calls for route {route.get('path', '')}: {str(e)}"
        )
        return False


def verify_api_calls_with_llm(
    controller_code: str,
    template_code: str,
    react_code: str,
) -> Optional[str]:
    """
    Use LLM to verify and potentially update the API calls of a React component.
    ...
    try:
        ...
        prompt = PROMPT_FOR_API_CALLS_VERIFICATION(
        ...
        logger.error(f"Error in LLM API calls verification: {str(e)}")
        return None

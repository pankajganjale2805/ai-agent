...
class AngularToNextMigrator:
    def __init__(
        ...
        if not self.angular_root.exists() or not self.angular_root.is_dir():
            raise ValueError(
                f"Angular project root {angular_root} does not exist or is not a directory"
            )
    ...
    def analyze_controllers(self) -> None:
            ...
            try:
                ...
                for directive_name in directive_matches:
                    if "controller:" in content:
                        ...
                        relative_path = file_path.relative_to(self.angular_root)
                        self.controllers[f"Directive_{directive_name}"] = str(
                            relative_path
                        )
            except Exception as e:
        ...
        print(
            f"Found {len(self.controllers)} controllers and directives with controllers"
        )
        ...
        for file_path in service_files:
            try:
                ...
                service_matches = re.findall(
                    r"\.(?:service|factory|provider)\(['\"]([^'\"]+)['\"]", content
                )
    ...
    def analyze_dependencies(self) -> None:
        ...
        for dep_name in dependencies.keys():
            ...
            if (
                clean_name in self.dependency_mapping
                and self.dependency_mapping[clean_name]
            ):
        ...
        if any(
            lib in dependencies for lib in ["angular-material", "material-design-lite"]
        ):
        ...
        print(
            f"Identified {len(self.react_dependencies)} React dependencies based on Angular libraries"
        )
    ...
    def update_package_json(self) -> None:
        ...
        try:
            ...
            if "sass" in self.react_dependencies:
                package_data["devDependencies"]["sass"] = self.react_dependencies[
                    "sass"
                ]
            ...
            print(
                f"Updated package.json with {len(self.react_dependencies)} dependencies"
            )
        except Exception as e:
    ...
    def generate_next_pages(self) -> None:
        ...
        try:
            ...
            print(
                f"Successfully updated routes with path property in scripts/admin-analysis-report.json"
            )
        except Exception as e:
    ...
    def _generate_page_for_route(self, route: Dict) -> None:
        ...
        prompt = prompts._get_generate_page_for_route_prompt(
            controller_code, template_code, component_name
        )

        # Use convert_with_llm to perform the conversion
        react_component = self.convert_with_llm(
            prompt, "controller_template", "react_component"
        )
    ...
    def _generate_default_component(self, component_name: str, url: str) -> str:
        ...
        template += (
            f'      <h1 className="text-3xl font-bold mb-6">{component_name}</h1>\n'
        )
        template += "      \n"
    ...
    def convert_with_llm(
        self, source_code: str, source_type: str, target_type: str
    ) -> str:
        """
        ...
        if not self.api_key:
            print(
                "Warning: No OpenRouter API key provided. Skipping LLM-based conversion."
            )
            return None
        ...
        try:
            ...
            print(
                f"Sending request to {self.model} for {source_type} to {target_type} conversion..."
            )
    ...
    def _clean_jsx_component(self, component_code: str, component_name: str) -> str:
        ...
        if (
            (
                "useState" in component_code
                or "useEffect" in component_code
                or "useRouter" in component_code
            )
            and not "'use client'" in component_code
            and not '"use client"' in component_code
        ):
        ...
        has_typescript = (
            "interface " in component_code
            or "type " in component_code
            or ": React." in component_code
            or "<any>" in component_code
            or ": string" in component_code
        )
        ...
        if not has_typescript:
            ...
            func_pattern = (
                r"(export\s+(?:default\s+)?function\s+" + component_name + r"\s*\()"
            )
            ...
            if func_match:
                ...
                if "props" in component_code and not "props: " in component_code:
                    type_replacement = (
                        f"{func_match.group(1)}props: {component_name}Props"
                    )
                    component_code = re.sub(
                        func_pattern, type_replacement, component_code
                    )
                    ...
                    if not f"interface {component_name}Props" in component_code:
                        ...
                        last_import = re.search(
                            import_pattern, component_code, re.DOTALL
                        )
                        if last_import:
                            component_code = re.sub(
                                import_pattern,
        ...
        if (
            "useState" in component_code
            or "useEffect" in component_code
            or "useRef" in component_code
        ):
            if (
                not "import React" in component_code
                and not "import { " in component_code
            ):
                hooks = []
        ...
        if (
            "useRouter" in component_code
            and not "import { useRouter }" in component_code
        ):
            ...
            if f"function {component_name}" in component_code:
                ...
                if func_match:
                    component_code = component_code.replace(
                        func_match.group(0), f"export default {func_match.group(0)}"
                    )
            else:
...
"""
        ...
        for service_name, service_path in self.services.items():
            try:
                ...
                if any(
                    name in service_name.lower()
                    for name in ["http", "util", "common", "auth", "helper", "service"]
                ):
                    continue
                ...
                api_name = re.sub(
                    r"([a-z0-9])([A-Z])", r"\1-\2", api_name
                ).lower()  # Convert to kebab-case
                ...
                route_content = self._convert_service_to_api_route(
                    service_name, service_content, api_name
                )
    ...
    def _convert_service_to_api_route(
        self, service_name: str, service_content: str, api_name: str
    ) -> str:
        """
        ...
        prompt = prompts._get_api_service_prompt(
            service_content, service_name, api_name
        )
...
"""
    ...
    def _generate_default_api_handler(
        self, service_name: str, api_name: str, api_dir: Path
    ) -> None:
        """
    ...
    def _create_style_imports(self, styles_dir: Path) -> None:
        ...
        if css_files:
            ...
            if css_content:
                ...
                print(
                    f"Created {global_css} with content from {len(css_files)} CSS files"
                )
...
export default function Home() {
  ...
  return (
    ...
    def migrate(self) -> None:
        ...
        print(
            f"Migration completed successfully! Next.js project is available at {self.nextjs_root}"
        )
    ...
    def convert_directives_to_components(self) -> None:
        ...
        for directive_name, directive_path in self.directives.items():
            try:
                ...
                component_name = "".join(
                    part.capitalize()
                    for part in re.findall(r"[A-Za-z][a-z]*", directive_name)
                )
                ...
                print(
                    f"Converting directive: {directive_name} to component: {component_name}"
                )
...
"""
                ...
                react_component = self.convert_with_llm(
                    prompt, "directive", "react_component"
                )

                if not react_component:
                    print(
                        f"Warning: Failed to convert directive {directive_name}. Creating a default component instead."
                    )
                    # Generate a simple component scaffold
                    react_component = self._generate_default_component_scaffold(
                        component_name
                    )
                ...
                converted_count += 1
                print(
                    f"Created React component for directive {directive_name}: {output_file}"
                )
...
"""
                    ...
                    if not react_hook:
                        print(
                            f"Warning: Failed to convert service {service_name}. Creating a default hook instead."
                        )
                        # Generate a simple hook scaffold
                        react_hook = self._generate_default_hook_scaffold(
                            hook_name, service_name
                        )
                    ...
                    converted_count += 1
                    print(
                        f"Created React hook for service {service_name}: {output_file}"
                    )

                except Exception as e:
                    print(
                        f"Warning: Could not convert service {service_name} to hook: {e}"
                    )
                    traceback.print_exc()
...
"""
            ...
            updated_layout = updated_layout.replace(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--angular-root", required=True, help="Path to Angular project root"
    )
    parser.add_argument(
        "--nextjs-root", required=True, help="Path to output Next.js project"
    )
    parser.add_argument(
        "--api-key",
        help="OpenRouter API key (optional, will use env var if not provided)",
    )
    parser.add_argument(
        "--model", help="LLM model to use (default: google/gemini-2.0-flash-001)"
    )
    parser.add_argument(
        "--analysis-file",
        help="Path to pre-computed analysis JSON file (optional, will look in scripts/ if not provided)",
    )
